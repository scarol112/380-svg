...
def compute_bounding_box(elements: list[PlacedElement]) -> tuple[float, float, float, float]:
    ...
    for e in geometry:
        if e.kind == "circle":
            ...
            continue
        if e.kind == "arc":
            r = e.extra.get("radius", 0.0)
            start_deg = e.extra.get("start_angle", 0.0)
            sweep_deg = e.extra.get("sweep", 90.0)
            ccw = e.extra.get("ccw", False)
            # compass → SVG: (start + 270) % 360
            start_rad = math.radians((start_deg + 270) % 360)
            end_rad = start_rad + math.radians(sweep_deg) * (-1 if ccw else 1)
            arc_xs = [e.x + r * math.cos(start_rad), e.x + r * math.cos(end_rad)]
            arc_ys = [e.y + r * math.sin(start_rad), e.y + r * math.sin(end_rad)]
            TWO_PI = 2 * math.pi
            for k in range(4):
                card_rad = math.radians(k * 90)
                dist = (card_rad - start_rad) % TWO_PI if not ccw else (start_rad - card_rad) % TWO_PI
                if dist <= math.radians(sweep_deg) + 1e-9:
                    arc_xs.append(e.x + r * math.cos(card_rad))
                    arc_ys.append(e.y + r * math.sin(card_rad))
            xs.extend(arc_xs)
            ys.extend(arc_ys)
            continue
