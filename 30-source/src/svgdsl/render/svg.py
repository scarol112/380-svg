...
def _element_bbox_px(
    ...
    if elem.kind == "arc":
        ...
        # include any cardinal points (0°, 90°, 180°, 270°) the arc passes through
        TWO_PI = 2 * math.pi
        for k in range(4):
            ...
            if dist <= math.radians(sweep_deg) + 1e-9:
                arc_xs.append(x0 + r * math.cos(card_rad))
                arc_ys.append(y0 + r * math.sin(card_rad))
        pad = max(elem.lw / 2, 1.0)
        return min(arc_xs) - pad, min(arc_ys) - pad, max(arc_xs) + pad, max(arc_ys) + pad
